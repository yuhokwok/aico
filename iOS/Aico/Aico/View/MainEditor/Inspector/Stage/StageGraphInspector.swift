    ...
    var body: some View {
        ScrollView {
            VStack (alignment: .leading) {
                ...
                VStack(alignment: .leading) {
                    InspectorSectionTitle("Task Name")
                    
                    HStack {
                        TextField("", text: $name, prompt: Text("hello"))
...
//                        
    ...
    @MainActor
    func commitChange(){
        if let id = editorState.selectedId, var stageGraph = handler.graph(for: id) as? StageGraph {
