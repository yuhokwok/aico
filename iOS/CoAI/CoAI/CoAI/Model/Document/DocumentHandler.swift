...
class DocumentHandler : ObservableObject {
    ...
    //the document
    var document : CoAIProject?
    
    
    //the project
    @Published var project : Project = .new()
    
    
    @Published var isReady : Bool = false

    //var projectManager : ProjectManager
    init(document : CoAIProject?, generatedProject : GeneratedProject? = nil) {
        ...
        if document != nil {
            Task {
                ...
                DispatchQueue.main.async {
                    if isReady == true {
                        if var project = document?.project {
                            if let generatedProject = generatedProject {
                                project.name = generatedProject.projectName
                                project.projectGraph.nodes = []
                                for step in generatedProject.steps {
                                    ...
                                    project.projectGraph.nodes.append(stage)
                                }
                    ...
                    self.project.editorState.selectedStageId = self.project.projectGraph.nodes.first?.id ?? ""
                    self.isReady = true
                }
            }
        } else {
    ...
    @MainActor func deleteEntity(with identifier : String) {
        ...
        if var graph = graph as? RelationshipGraph {
            ...
            if let target = entity as? Channel {
                ...
                for (index, node) in graph.nodes.enumerated() {
                    if entity.identifier == node.identifier {
                        editorState.selectedStageId = graph.nodes[index == 0 ? (index + 1) : (index - 1)].id
                        graph.nodes.remove(at: index)
                        break
                    }
                }
            ...
            guard project.projectGraph.nodes.count > 1 else {
                return
            }
